...
- `/registro` crea PERITOS, no clientes: `crm_user` (rol 'perito') + fila en `expert` (directorio /crm/experts, estado 'candidato', clasificado automáticamente por nivel junior/senior/master + macro-categoría según formación+experiencia vía `src/lib/peritos/`) + `registro_peritus` (vinculado por `user_id`→crm_user). NO crea `crm_client`. El portal del perito muestra los casos donde es `cases.assigned_expert_id` (ownership: `verifyExpertOwnsCase`). El `role:'cliente'` del JWT es etiqueta interna del portal, no el rol del crm_user.
